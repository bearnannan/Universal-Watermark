...
data class OverlayConfig(
    ...
    fun getFormattedText(): String {
        return buildString {
            ...
            if (projectName.isNotEmpty()) append("$projectName\n")
            if (inspectorName.isNotEmpty()) append("$inspectorName\n")
...
object OverlayUtils {
    ...
    fun getFormattedDate(date: Date, pattern: String, useThaiLocale: Boolean): String {
        ...
        // Force Gregorian Calendar so that simpleDateFormat ALWAYS outputs the AD year initially.
        // This prevents the OS from pre-converting to Buddhist year, which breaks the replacement logic.
        val gregorianCalendar = java.util.Calendar.getInstance(java.util.TimeZone.getDefault(), Locale.US)
        ...
        if (useThaiLocale) {
            val yearAD = gregorianCalendar.get(java.util.Calendar.YEAR)
            ...
            if (pattern.contains("yyyy")) {
                ...
                // Replace the last occurrence to avoid accidentally replacing a day or month that matches the year
                // e.g. "26/06/26" -> replace the last "26" -> "26/06/69"
                val lastIndex = formattedDate.lastIndexOf(shortYearAD)
                if (lastIndex != -1) {
                    formattedDate = formattedDate.substring(0, lastIndex) + shortYearBE + formattedDate.substring(lastIndex + shortYearAD.length)
                }
            }
        }
